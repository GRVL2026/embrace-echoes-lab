    ...
    <div className="min-h-screen w-full bg-background text-foreground">
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 py-6 sm:py-8">
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
          <div>
            <h2 className="font-display text-xl sm:text-2xl font-bold">Synchronisation Cegid</h2>
            <p className="text-sm text-muted-foreground">
              Diagnostic de connexion et synchronisation des flux OData Cegid XRP Flex.
            </p>
          </div>
        ...
        {lastLogs.length > 0 && (
          <div className="mb-6 rounded-lg border border-border bg-card/40 p-4">
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {lastLogs.map((l) => (
                <div key={l.feed} className="flex items-center justify-between rounded border border-border/60 bg-background/40 px-3 py-2 text-sm">
                  <div className="flex items-center gap-2">
                    {l.ok ? <CheckCircle2 className="h-4 w-4 text-secondary" /> : <XCircle className="h-4 w-4 text-destructive" />}
                    <span className="font-medium">{l.feed}</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                    <span>{l.rows_loaded ?? 0} lignes</span>
                    {l.finished_at && <span>· {new Date(l.finished_at).toLocaleString("fr-FR")}</span>}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mb-6 rounded-lg border border-border bg-card/40 p-4">
            ...
            <div className="space-y-2">
              {FEEDS.map((feed) => {
                ...
                return (
                  <div key={feed} className="rounded border border-border/60 bg-background/40 px-3 py-2 text-sm">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-3 text-xs">
                        ...
                        <Button type="button" variant="outline" size="sm" className="h-7 px-2 text-xs" onClick={() => retryFeed(feed)} disabled={syncing || running || status === "running"}>
        ...
        {summary && summary.length > 0 && (
          <div className="mb-6 rounded-lg border border-border bg-card/40 p-4">
            ...
            <div className="space-y-2">
              {summary.map((s) => (
                <div key={s.feed} className="flex items-center justify-between rounded border border-border/60 bg-background/40 px-3 py-2 text-sm">
                  <div className="flex items-center gap-2">
                    {s.ok ? <CheckCircle2 className="h-4 w-4 text-secondary" /> : <XCircle className="h-4 w-4 text-destructive" />}
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    ...
                    {s.error && <span className="max-w-md truncate text-destructive" title={s.error}>{s.error}</span>}
                  </div>
                </div>
              ))}
            </div>
        ...
        {diag && (
          <div className="space-y-4">
            <div className="rounded-lg border border-border bg-card/40 p-4">
              <div className="mb-3 flex items-center gap-2">
                {diag.token_step.ok ? <CheckCircle2 className="h-4 w-4 text-secondary" /> : <XCircle className="h-4 w-4 text-destructive" />}
                <h3 className="font-display text-lg font-semibold">Ticket OAuth</h3>
                <Badge variant={diag.token_step.ok ? "default" : "destructive"}>HTTP {diag.token_step.http_status ?? "—"}</Badge>
                <Badge variant="outline">{diag.token_step.duration_ms} ms</Badge>
              </div>
              {diag.token_step.error && (
                <div className="rounded border border-destructive/40 bg-destructive/10 p-2 text-sm text-destructive">{diag.token_step.error}</div>
              )}
              {diag.token_step.preview && (
                <div className="mt-2">
                  <div className="mb-1 text-xs font-semibold text-muted-foreground">Aperçu réponse</div>
                  <pre className="max-h-48 overflow-auto rounded bg-muted/50 p-3 text-xs whitespace-pre-wrap">{diag.token_step.preview}</pre>
                </div>
              )}
            </div>

            {diag.feeds.map((r) => (
              <div key={r.name} className="rounded-lg border border-border bg-card/40 p-4">
                <div className="mb-3 flex items-center gap-2">
                  {r.ok ? <CheckCircle2 className="h-4 w-4 text-secondary" /> : <XCircle className="h-4 w-4 text-destructive" />}
                  <h3 className="font-display text-lg font-semibold">{r.name}</h3>
                  <Badge variant={r.ok ? "default" : "destructive"}>HTTP {r.http_status ?? "—"}</Badge>
                  <Badge variant="outline">{r.duration_ms} ms</Badge>
                  {r.format && <Badge variant="outline">{r.format}</Badge>}
                </div>
                <div className="mb-2 truncate text-xs text-muted-foreground">{r.url}</div>
                {r.error && (
                  <div className="rounded border border-destructive/40 bg-destructive/10 p-2 text-sm text-destructive">{r.error}</div>
                )}
                {r.columns && r.columns.length > 0 && (
                  <div className="mb-3 mt-2">
                    <div className="mb-1 text-xs font-semibold text-muted-foreground">Colonnes ({r.columns.length})</div>
                ...
                {r.sample && r.sample.length > 0 && (
                  <div>
                    <div className="mb-1 text-xs font-semibold text-muted-foreground">Échantillon ({r.sample.length})</div>
                    <pre className="max-h-72 overflow-auto rounded bg-muted/50 p-3 text-xs">{JSON.stringify(r.sample, null, 2)}</pre>
                  </div>
                )}
                {r.preview && (
                  <div>
                    <div className="mb-1 text-xs font-semibold text-muted-foreground">Aperçu brut</div>
                    <pre className="max-h-72 overflow-auto rounded bg-muted/50 p-3 text-xs whitespace-pre-wrap">{r.preview}</pre>
                  </div>
                )}
              </div>
            ))}
    ...
    <div className={`mb-6 rounded-lg border p-4 ${stale ? "border-amber-500/50 bg-amber-500/5" : "border-border bg-card/40"}`}>
      ...
      {stale && (
        <p className="mt-3 text-xs text-amber-600">
          ⚠ Aucune synchro réussie depuis plus de 36h — la sentinelle a créé une alerte.
        </p>
      )}
    </div>
  );
