-- Cockpit prospection — tags propriétaire sur la carte.
-- Enrichit get_map_points : chaque prospect renvoie désormais son commercial
-- (proprietaire uuid + proprietaire_nom résolu via profiles), pour colorer /
-- étiqueter les marqueurs par commercial et filtrer la carte par propriétaire.
-- Signature inchangée (jsonb) → CREATE OR REPLACE. Reste réservé admin/direction.
