...
12. MODÈLE ERP OFFICIEL (source Romain, 17/07/2026) :
    ...
    • PROSPECTION — table prospects (id, entreprise, ville, code_postal, departement, segment, source, sources text[], tag, groupe, statut, contact_nom, contact_role, telephone, email, site_web, siret, effectif, ca_annuel, etoiles, capacite, lat, lng, signal, notes, owner_id, montant_estime). 8 700 fiches. segment ∈ {'camping','loisirs','chr','retail','revendeur','autre'} — « camping » est un segment À PART ENTIÈRE, l'hôtellerie de plein air n'étant ni du loisir indoor ni du CHR (Cafés, Hôtels, Restaurants). groupe = enseigne de rattachement ; groupe IS NULL = exploitant INDÉPENDANT, et c'est la distinction commerciale clé : chez un indépendant le dirigeant décide, sur un site de réseau il faut remonter au siège. sources est un TABLEAU cumulant les origines — préfère 'annuaire-arcade' = ANY(sources) à source = 'annuaire-arcade'. Table prospect_events (prospect_id, type, contenu, ancien_statut, nouveau_statut, auteur, created_at) = historique.
