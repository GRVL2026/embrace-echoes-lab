-- Permettre aux appelants (authenticated / service_role / anon) de basculer sur copilot_readonly
GRANT copilot_readonly TO authenticated, service_role, anon;

-- Redéfinir en SECURITY INVOKER (SET LOCAL ROLE est autorisé dans ce mode)
...
GRANT EXECUTE ON FUNCTION public.gaia_query_restricted(text) TO PUBLIC;